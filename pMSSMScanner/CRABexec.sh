...
SEUSERSUBDIR=pMSSM_Interpretation_HugeFullScan
...
MaxTime=1 ## maximum time for a crab job (in minutes)
...
echo "NOTE THAT WE'RE ONLY LOOKING AT 10 MODELS ATM"
for model in {1..10000}; do
  ...
  ./MicroSuggestion.exec `pwd` > MicroLog.txt
...
mv fout.root Candidates/Overview.root
